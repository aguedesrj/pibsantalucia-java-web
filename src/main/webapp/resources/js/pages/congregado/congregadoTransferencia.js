$(document).ready(function() {
	...
	// máscaras
	$('#traDtTransferencia').mask('00/00/0000', {
		reverse: true
	});
	...
	$('#pesNome').typeahead({
        ...
        source: function (query, process) {
    			$.ajax({
           		url: 'TransferenciaPesquisaNome.action',
    ...
    $('#formCongregado').formValidation({
        framework: 'bootstrap',
        fields: {
        		'transferenciaVO.viewMembroVO.pesNome': {
	    			validators: {
	                notEmpty: {
        		...
        		'transferenciaVO.traDtTransferencia': {
        			validators: {
                    notEmpty: {
                        message: $("#ERRO_TRANSFERENCIA_DATA_OBRIGATORIO").val()
                    }
                }
            },
        		'transferenciaVO.traMotivo': {
        			validators: {
                    notEmpty: {
                        message: $("#ERRO_TRANSFERENCIA_MOTIVO_OBRIGATORIO").val()
                    }
                }
            },
	    		'transferenciaVO.traStatus': {
	    			validators: {
	                notEmpty: {
	                    message: $("#ERRO_TRANSFERENCIA_STATUS_OBRIGATORIO").val()
	                }
	            }
	        },           
	    		'transferenciaVO.traNomeIgreja': {
	    			validators: {
	                notEmpty: {
	                    message: $("#ERRO_TRANSFERENCIA_NOME_IGREJA_OBRIGATORIO").val()
	                }
	            }
	        }            
        }
	    			...
	    			BootstrapDialog.show({
	    		        ...
	    		        buttons: [{
	    		            ...
	    		            action: function(dialogItself){
	    		                dialogItself.close();
	    		                window.location = "Transferencia.action";
	    		            }
	    		        }]
	    		    });
       		},
