$(document).ready(function() {
	...
	// máscaras
	$('#desDtDesligamento').mask('00/00/0000', {
		reverse: true
	});
	...
	$('#pesNome').typeahead({
        ...
        source: function (query, process) {
    			$.ajax({
           		url: 'DesligamentoPesquisaNome.action',
    ...
    $('#formCongregado').formValidation({
        framework: 'bootstrap',
        fields: {
        		'desligamentoVO.viewMembroVO.pesNome': {
	    			validators: {
	                notEmpty: {
        		...
        		'desligamentoVO.desDtDesligamento': {
        			validators: {
                    notEmpty: {
                        message: $("#ERRO_DESLIGAMENTO_DATA_OBRIGATORIO").val()
                    }
                }
            }
        }
	    			...
	    			BootstrapDialog.show({
	    		        ...
	    		        buttons: [{
	    		            ...
	    		            action: function(dialogItself){
	    		                dialogItself.close();
	    		                window.location = "Desligamento.action";
	    		            }
	    		        }]
	    		    });
       		},
